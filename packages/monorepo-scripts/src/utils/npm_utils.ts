...
const NPM_REGISTRY_BASE_URL = JSON.parse(fs.readFileSync('lerna.json').toString()).registry;
...
export const npmUtils = {
    async getPackageRegistryJsonIfExistsAsync(packageName: string): Promise<PackageRegistryJson | undefined> {
        ...
        if (response.status === NOT_FOUND_STATUS) {
            ...
            throw new Error(
                `Request to ${url} failed. Check your internet connection and that ${NPM_REGISTRY_BASE_URL} is up.`,
            );
        }
